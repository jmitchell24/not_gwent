...
#include "rlImGui/imgui/imgui.h" // for imGui::GetCurrentWindow()
...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (ImGui::IsWindowFocused())
	{
		...
		else if (!IsReadOnly() && !ctrl && !alt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Tab)))
        {
            ...
            InsertText(std::string(mTabSize, ' '));
        }
...
bool TextEditor::ConsumeChangedText(std::string& aText)
{
	if (IsTextChanged())
	{
		aText = std::move(GetText());
		mTextChanged = false;
		return true;
	}
	return false;
...
int TextEditor::GetSelectionLength() const
{
    int len = mState.mSelectionEnd.mColumn - mState.mSelectionStart.mColumn;
    return len;
}
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::SQL()
{
	...
	if (!inited)
	{
		static const char* const keywords[] = {
			...
			"DUMMY_JMP", "OPENXML", "WAITFOR", "DUMP", "OPTION", "WHEN", "ELSE", "OR", "WHERE", "END", "ORDER", "WHILE", "ERRLVL", "OUTER", "WITH", "ESCAPE", "OVER", "WRITETEXT"
		};
